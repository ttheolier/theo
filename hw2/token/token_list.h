...
public:
    ...
    //Helper function to return Item pointer of desired Item
    std::shared_ptr<Item> find(size_t index);

    //Helper to link two nodes together, smaller index 
    void link(std::shared_ptr<Item> ind1, std::shared_ptr<Item> ind2);
