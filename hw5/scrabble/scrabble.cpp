...
{
    //SHOULD THIS HAVE VARIABLE HAND SIZE??
    ...
    for (unsigned int i=0;i<numplay;i++)
    {
        ...
        cout<<"Is this a computer player, y/n?"<<endl;
        char type;
        cin>>type;
        if (cin.fail())
        {
        ...
        if (type == 'y' || type == 'Y')
        {
            players.push_back(make_shared<ComputerPlayer>(ComputerPlayer(player,this->hand_size)));
            vector<TileKind> draw = tile_bag.remove_random_tiles(this->hand_size);
            players.back()->add_tiles(draw);
        }
        else
        {
            players.push_back(make_shared<HumanPlayer>(HumanPlayer(player,this->hand_size)));
            vector<TileKind> draw = tile_bag.remove_random_tiles(this->hand_size);
            players.back()->add_tiles(draw);
            num_human++;
        }
